...
glm::vec3 readvoxvert(int x,int y,int z,uint8_t a,uint8_t b,uint8_t c,int dep) {
    return glm::vec3(x-ASBLOCKLOC(0)+(a/512.0)*(1<<dep),y-ASBLOCKLOC(0)+(b/512.0)*(1<<dep),z-ASBLOCKLOC(0)+(c/512.0)*(1<<dep));
}
uint8_t savevoxvert(int x,float a,int dep) {
    return (a-((x-ASBLOCKLOC(0))&~((1<<(dep+1))-1)))/512.0;
}
...
OctreePortionAwareBranch::OctreePortionAwareBranch(OctreeSegment* a,OctreeSegment* b,
                                                   OctreeSegment* c,OctreeSegment* d,
                                                   OctreeSegment* e,OctreeSegment* f,
                                                   OctreeSegment* g,OctreeSegment* h,bool hl,glm::vec3 po) :
OctreeBranch(a,b,c,d,e,f,g,h,CHPOWER-1,po),hardload(hl) {}//bakeddetails=WORLDFILE_VBITMASK;}
OctreePortionAwareBranch::OctreePortionAwareBranch(BlockId bi,glm::vec3 glm,uint8_t con) : OctreeBranch(bi,CHPOWER-1,glm,con),hardload(false) {}
...
OctreeBranch::OctreeBranch(OctreeSegment* a,OctreeSegment* b,
                           OctreeSegment* c,OctreeSegment* d,
                           OctreeSegment* e,OctreeSegment* f,
                           OctreeSegment* g,OctreeSegment* h, int dep,glm::vec3 pof) : depth(dep) {
...
OctreeBranch::OctreeBranch(BlockId a,int dep,glm::vec3 pof,uint8_t conn) : depth(dep),connections(conn) {
//    point = glm::vec3(x*(1<<depth)/128.0,y*(1<<depth)/128.0,z*(1<<depth)/128.0);
    point = (int)g_vertecies[depth+1].size();
    g_vertecies[depth+1].push_back(pof);
        ...
        for (BlockLoc yi=y;yi<y+add;yi++) {
            for (BlockLoc zi=z;zi<z+add;zi++) {
                if (data[xi][yi][zi] != equi) {
                    ...
                    if (recur == 7) {return new OctreePortionAwareBranch(a,b,c,d,e,f,g,h,true);}
